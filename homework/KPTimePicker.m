...
#define kClockLabelFont [UIFont fontWithName:@"HelveticaNeue-Light" size:valForScreen(45,65)]
...
#define kDefLightColor          color(251,223,187,1) //tcolor(SearchDrawerColor)
#define kDefDarkColor           color(70,84,198,1)
...
-(void)pressedForwardButton:(UIButton*)sender{
    [self.delegate timePicker:self selectedDate:[NSDate date]];
}
...
- (CGPoint)pointFromPoint:(CGPoint)origin withDistance:(float)distance towardAngle:(float)angle
{
    ...
    return CGPointMake(origin.x + distance * cos(radAngle), (origin.y + distance * sin(radAngle))+40);
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.forwardButton.frame = CGRectMake(self.bounds.size.width-kBackMargin-kBackButtonSize, self.bounds.size.height-kBackButtonSize-kBackMargin, kBackButtonSize, kBackButtonSize);
        self.forwardButton.autoresizingMask = (UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleTopMargin);
        [self.forwardButton setImage:[UIImage imageNamed:@"round_forwardarrow_big"] forState:UIControlStateNormal];
        [self.forwardButton setImage:[UIImage imageNamed:@"round_forwardarrow_big-high"] forState:UIControlStateHighlighted];
        [self.forwardButton addTarget:self action:@selector(pressedForwardButton:) forControlEvents:UIControlEventTouchUpInside];
        ...
        //DAY LABEL
        self.dayLabel = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 120)];
        self.dayLabel.backgroundColor = [UIColor clearColor];
        [self.dayLabel setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [self.dayLabel setTitleColor:[UIColor lightGrayColor] forState:UIControlStateHighlighted];
        self.dayLabel.titleLabel.font = kDayLabelFont;
        self.dayLabel.titleLabel.textAlignment = NSTextAlignmentCenter;
...
-(void)layoutSubviews{
    ...
    self.dayLabel.frame = CGRectMake(0, spacing+heightForDay+heightForTime+15, self.bounds.size.width, heightForDay);
    self.clockLabel.frame = CGRectMake(0, spacing+heightForDay+15, self.bounds.size.width, heightForTime);
