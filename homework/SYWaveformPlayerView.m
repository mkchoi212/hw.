...
- (void)layoutSubviews {
    ...
    playPauseButton.frame = CGRectMake(5, self.frame.size.height/2+5 - self.frame.size.height/4 , self.frame.size.height/2-8, self.frame.size.height/2-8);
    playPauseButton.layer.cornerRadius = self.frame.size.height/4;
    
    waveformView.frame = CGRectMake(self.frame.size.height/2 + 10, 0, self.frame.size.width - (self.frame.size.height/2-5), self.frame.size.height);
}
